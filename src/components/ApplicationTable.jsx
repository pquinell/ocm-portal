...
export default function ApplicationTable({ applications, isLoading, onSelect, selectedId }) {
  ...
  const SortIcon = ({ col }) => {
    ...
    return <span className="text-white/60 ml-1">{sortDir === 'asc' ? '↑' : '↓'}</span>;
  };
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={6} className="px-4 py-16 text-center text-white/20 text-sm">
              ...
              sorted.map(app => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ${TYPE_STYLES[app.type] ?? ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="text-sm font-medium text-white">{app.applicantName}</div>
                        <div className="text-xs text-white/30 font-mono">{app.applicationId?.slice(0, 8)}...</div>
                      </div>
                    </div>
                  </td>
                  ...
                  {/* Submitted */}
                  <td className="px-4 py-3">
                    <span className="text-sm text-white/30">{formatDate(app.submittedAt)}</span>
                  </td>
                </tr>
              ))
            )}
